import networkx as nx
import matplotlib.pyplot as plt
G1 = nx.Graph()
edges1 = [
    ('A','B','e1'), ('B','C','e2'), ('C','D','e3'), ('D','A','e4'),
    ('D','H','e5'), ('H','G','e6'), ('H','E','e7'),
    ('G','F','e8'), ('C','G','e9'), ('E','F','e10'),
    ('A','E','e11'), ('B','F','e12')
]
for u, v, e in edges1:
    G1.add_edge(u, v, label=e)
pos1 = {
    'A': (2, 2), 'B': (2, 0), 'C': (0, 0), 'D': (0, 2),
    'E': (1.5, 1.5), 'F': (1.5, 0.5),
    'G': (0.5, 0.5), 'H': (0.5, 1.5)
}
hamiltonian1 = ['A', 'B', 'C', 'D', 'H', 'G', 'F', 'E', 'A']
G2 = nx.Graph()
edges2 = [
    ('A', 'F','e1'), ('A', 'B','e2'), ('B', 'C','e3'),
    ('C', 'D','e4'), ('D', 'E','e5'), ('E', 'F','e6'),
    ('F', 'G','e7'), ('A', 'E','e8'), ('E', 'G','e9'),
    ('A', 'G','e10'), ('B', 'D','e11'), ('G', 'C','e12'),
    ('F','B','e13'), ('F','D','e14'), ('F','C','e15')
]
for u, v, e in edges2:
    G2.add_edge(u, v, label=e)
pos2 = {
    'A': (1, 2), 'B': (3, 2), 'C': (4, 1),
    'D': (3, 0), 'E': (1, 0), 'F': (0, 1), 'G': (2, 1)
}
hamiltonian2 = ['A', 'B', 'C', 'D', 'E', 'G', 'F', 'A']
def draw_hamiltonian_steps(G, pos, circuit, title):
    steps = len(circuit) - 1
    cols = 3
    rows = (steps + cols - 1) // cols
    plt.figure(figsize=(18, 10))
    plt.suptitle(title, fontsize=20)
    for i in range(steps):
        plt.subplot(rows, cols, i + 1)
        nx.draw(
            G,
            pos,
            with_labels=True,
            node_color='lightblue',
            node_size=1200,
            width=2
        )
        edge_labels = {(u, v): d['label']
                       for u, v, d in G.edges(data=True)}
        nx.draw_networkx_edge_labels(
            G,
            pos,
            edge_labels=edge_labels,
            font_size=10
        )
        path_edges = []
        for j in range(i + 1):
            u = circuit[j]
            v = circuit[j + 1]
            path_edges.append((u, v))
        nx.draw_networkx_edges(
            G,
            pos,
            edgelist=path_edges,
            edge_color='red',
            width=4
        )
        visited = circuit[:i + 2]
        plt.title(
            f"Step {i+1}\nVisited: {visited}",
            fontsize=14
        )
        plt.axis('off')
    plt.tight_layout()
    plt.show()
draw_hamiltonian_steps(
    G1,
    pos1,
    hamiltonian1,
    "Hamiltonian Circuit Step-by-Step : Graph (a)"
)
draw_hamiltonian_steps(
    G2,
    pos2,
    hamiltonian2,
    "Hamiltonian Circuit Step-by-Step : Graph (b)"
)